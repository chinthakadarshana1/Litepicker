...
Litepicker.prototype.setDateRange = function (
    date1,
    date2,
    force: boolean = false,
) {
    ...
    const d1 = new DateTime(date1, this.options.format, this.options.lang);
    const d2 = new DateTime(date2, this.options.format, this.options.lang);
    let isLocked = false;
    if (this.options.disallowLockDaysInRange) {
        isLocked = rangeIsLocked([d1, d2], this.options);
    } else {
        isLocked =
            dateIsLocked(d1, this.options, [d1, d2]) ||
